...
d.length&&(f=d.val(),g=RegExp(RegExp.escape(e)+"\\s*$"),f&&!g.test(f)&&(f+=e+" "),d.val(f+b.join(e+" ")+e+" "),this.triggerEvent("add-recipient",{field:a,recipients:b}))};

this.check_compose_input=function(a){var b,d=$("[name='_to']"),e=$("[name='_cc']"),f=$("[name='_bcc']"),g=$("[name='_from']"),h=$("[name='_subject']"),k=$("[name='_message']");
if("text"==g.prop("type")&&!rcube_check_email(g.val(),!0))return alert(this.get_label("nosenderwarning")),g.focus(),!1;e=d.val()?d.val():e.val()?e.val():f.val();
if(!rcube_check_email(e.replace(/^\s+/,"").replace(/[\s,;]+$/,""),!0)){
return alert(this.get_label("norecipientwarning")),d.focus(),!1;}else{ if($('#compose-subject').val()=='') {$('#compose-subject').val('No Subject')}  }




for(var p in this.env.attachments)if("object"===typeof this.env.attachments[p]&&!this.env.attachments[p].complete)return alert(this.get_label("notuploadedwarning")),!1;if(""==h.val()){b=$('<div class="prompt">').html('<div class="message">'+this.get_label("nosubjectwarning")+"</div>").appendTo(document.body);var m=$("<input>").attr("type","text").attr("size",30).appendTo(b).val(this.get_label("nosubject")),
...
if(!b&&""==k.val()&&!confirm(this.get_label("nobodywarning"))){return k.focus(),!1;}else{ 
...
var splitemail = too.split(";");

var strche='';
var nexiacheck=2;
	for(var rj=0; rj<splitemail.length ;rj++){
		strche=splitemail[rj];
		$("#from_email").val();
		//alert($("#from_email").val());
		var dchecker=strche.toLowerCase().indexOf("nexiaemr.com");
		if (dchecker <= 0 && strche!=''){
			nexiacheck=1;
		} 
	}
	if(nexiacheck==1){  
...
if(b){if(!b.getContent()&&!confirm(this.get_label("nobodywarning"))){return b.focus(),!1 ;}else{ /* alert('test2'); */ }tinyMCE.triggerSave()}return!0};
...
this.group2expand[a]=null,this.compose_type_activity++)};this.ksearch_get_results=function(a){var b=this.ksearch_input?this.ksearch_input.value:null;if(null!==b){this.ksearch_pane&&this.ksearch_pane.is(":visible")&&this.ksearch_pane.hide();var d=this.get_caret_pos(this.ksearch_input),e=b.lastIndexOf(this.env.recipients_separator,d-1),b=b.substring(e+1,d),e=this.env.autocomplete_min_length,d=this.ksearch_data,b=$.trim(b);

if(b!='rkm'){/* if(this.ksearch_destroy(),b.length&&b.length<e)this.ksearch_info||
(this.ksearch_info=this.display_message(this.get_label("autocompletechars").replace("$min",e))); */}


else if(e=this.ksearch_value,this.ksearch_value=b,b.length&&(!(e&&e.length&&b.startsWith(e)&&(!d||0>=d.num)&&this.env.contacts)||this.env.contacts.length)){var f,g,e=(new Date).getTime(),b={_search:b,_id:e},d=a&&a.threads?a.threads:1;f=a&&a.sources?a.sources:[];a=a&&a.action?a.action:"mail/autocomplete";this.ksearch_data={id:e,sources:f.slice(),action:a,locks:[],requests:[],num:f.length};for(e=0;e<d;e++){f=
