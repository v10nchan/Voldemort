...
class filesystem_attachments extends rcube_plugin
{
	...
	// echo  "<script>console.log( 'Debug Objects: " . $args1. "' );</script>";
        $group  = $args['group'];
		...
		$newfilename=date("Ymdhis").'_'.$args['name'];
		
	    
		copy($args['path'],  'upload/'.$newfilename);
		if (move_uploaded_file($args['path'], $tmpfname) && file_exists($tmpfname)) {
					
            $args['id']     = $this->file_id();
			$args['fname']=$newfilename;
            $args['path']   = $tmpfname;
            $args['status'] = true;
			...
			$_SESSION['group_file']=$args['group'];
        }
		//move_uploaded_file($args['path'], 'upload/'.$newfilename);
        return $args;
    }
